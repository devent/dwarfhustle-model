...
/**
 * @see ContainerObjectBuffer
 *
 * @author Erwin Müller, {@code <erwin@muellerpublic.de>}
 */
class ContainerObjectBufferTest {

    static Stream write_read_ContainerObject() {
        def args = []
        int offset = 0
    ...
    void write_read_ContainerObject(ByteBuffer b, int offset, long id, int kid, int oid, long map, int x, int y, int z, int w, int h, int d, int p, int t, int l, long m, long table, def expected) {
        ...
        ContainerObjectBuffer.getContainerObject(new UnsafeBuffer(b), offset, thato)
        assert thato.id == id
