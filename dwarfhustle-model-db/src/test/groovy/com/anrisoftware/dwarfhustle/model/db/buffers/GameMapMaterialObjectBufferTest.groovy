...
/**
 * @see GameMapMaterialObjectBuffer
 *
 * @author Erwin Müller, {@code <erwin@muellerpublic.de>}
 */
class GameMapMaterialObjectBufferTest {

    static Stream write_read_gameMapMaterialObject() {
        def args = []
        int offset = 0
    ...
    void write_read_gameMapMaterialObject(ByteBuffer b, int offset, long id, int kid, int oid, long map, int x, int y, int z, int w, int h, int d, int p, int t, int l, long m, def expected) {
        ...
        GameMapMaterialObjectBuffer.readMaterialObject(new UnsafeBuffer(b), offset, thato)
        assert thato.id == id
