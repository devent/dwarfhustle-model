...
class FurnitureBufferTest {
    ...
    @ParameterizedTest
    @MethodSource()
    void write_read_Furniture(ByteBuffer b, int offset, long id, int kid, int oid, long map, int x, int y, int z, int w, int h, int d, int p, int t, int l, long m, def expected) {
        def o = new Furniture(id, new GameBlockPos(x, y, z), m)
        o.size.x = w
        o.size.y = h
        o.size.z = d
        o.kid = kid
