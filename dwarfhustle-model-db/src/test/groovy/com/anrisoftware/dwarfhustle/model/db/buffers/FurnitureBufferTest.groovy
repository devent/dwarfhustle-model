 ...
 */
class FurnitureBufferTest {

    static Stream write_read_Furniture() {
        def args = []
        int offset = 0
        def b = ByteBuffer.allocate(offset + FurnitureBuffer.SIZE)
        ...
        offset = 3
        b = ByteBuffer.allocate(offset + FurnitureBuffer.SIZE)
    ...
    @ParameterizedTest
    @MethodSource()
    void write_read_Furniture(ByteBuffer b, int offset, long id, int kid, int oid, long map, int x, int y, int z, int w, int h, int d, int p, int t, int l, long m, int type, def expected) {
        def o = new Furniture(id, new GameBlockPos(x, y, z), m, type)
        o.size.x = w
        o.size.y = h
        o.size.z = d
        o.kid = kid
        ...
        o.temp = t
        o.lux = l
        FurnitureBuffer.setFurniture(new UnsafeBuffer(b), offset, o)
        assert HexFormat.of().formatHex(b.array()) == replace(expected, " ", "")
        def thato = new Furniture()
        FurnitureBuffer.getFurniture(new UnsafeBuffer(b), offset, thato)
        assert thato.id == id
