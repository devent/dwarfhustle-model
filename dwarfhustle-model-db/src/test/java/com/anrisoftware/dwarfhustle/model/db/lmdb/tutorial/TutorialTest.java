 ...
 */
package com.anrisoftware.dwarfhustle.model.db.lmdb.tutorial;
...
public final class TutorialTest {
     ...
     */
    @Test
    public void tutorial1(@TempDir File tmp) throws IOException {
        ...
        try (Txn<ByteBuffer> txn = env.txnRead()) {
            assertThat(db.get(txn, key), nullValue());
        }
     ...
     */
    @Test
    public void tutorial2(@TempDir File tmp) throws IOException, InterruptedException {
        final Env<ByteBuffer> env = createSimpleEnv(tmp);
        final Dbi<ByteBuffer> db = env.openDbi(DB_NAME, MDB_CREATE);
        final ByteBuffer key = allocateDirect(env.getMaxKeySize());
        final ByteBuffer val = allocateDirect(700);
    ...
    public void tutorial3(@TempDir File tmp) throws IOException {
        final Env<ByteBuffer> env = createSimpleEnv(tmp);
        final Dbi<ByteBuffer> db = env.openDbi(DB_NAME, MDB_CREATE);
        final ByteBuffer key = allocateDirect(env.getMaxKeySize());
        final ByteBuffer val = allocateDirect(700);
    ...
    @Test
    @SuppressWarnings("ConvertToTryWithResources")
    public void tutorial4(@TempDir File tmp) throws IOException {
        final Env<ByteBuffer> env = createSimpleEnv(tmp);
        final Dbi<ByteBuffer> db = env.openDbi(DB_NAME, MDB_CREATE);
    ...
    @Test
    @SuppressWarnings("ConvertToTryWithResources")
    public void tutorial5(@TempDir File tmp) throws IOException {
        final Env<ByteBuffer> env = createSimpleEnv(tmp);
    ...
    @Test
    @SuppressWarnings("ConvertToTryWithResources")
    public void tutorial6(@TempDir File tmp) throws IOException {
        // Note we need to specify the Verifier's DBI_COUNT for the Env.
        final Env<ByteBuffer> env = create(PROXY_OPTIMAL).setMapSize(10_485_760).setMaxDbs(Verifier.DBI_COUNT)
                .open(tmp);
    ...
    @Test
    @SuppressWarnings("ConvertToTryWithResources")
    public void tutorial7(@TempDir File tmp) throws IOException {
        ...
        final Env<DirectBuffer> env = create(PROXY_DB).setMapSize(10_485_760).setMaxDbs(1).open(tmp);
