...
public class TerrainCreateRules {
    ...
    public void initResources(Environment env) {
        MutableIntObjectMap<MutableIntList> materials = env.get(TerrainCreateKnowledge.MATERIALS_NAME);
        ...
        MutableIntIntMap objects = env.get(TerrainCreateKnowledge.OBJECTS_NAME);
    ...
    public void material_empty_set_oxygen(TerrainFact $f, RhsContext ctx) {
        $f.block.setMaterial(materialOxygen);
        ctx.update($f);
    }
    ...
    public boolean material_gas_test(int mid) {
        return gases.contains(mid);
    }

    public boolean material_liquid_test(int mid) {
        return liquids.contains(mid);
    }

    public boolean material_solid_test(int mid) {
